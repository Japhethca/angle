...
export type ReadOptionSetWithDescendantsInput = {
  slug: string;
};

export type ReadOptionSetWithDescendantsFields = UnifiedFieldSelection<OptionSetResourceSchema>[];
export type InferReadOptionSetWithDescendantsResult<
  Fields extends ReadOptionSetWithDescendantsFields,
> = Array<InferResult<OptionSetResourceSchema, Fields>>;

export type ReadOptionSetWithDescendantsResult<Fields extends ReadOptionSetWithDescendantsFields> = | { success: true; data: InferReadOptionSetWithDescendantsResult<Fields>; }
| { success: false; errors: AshRpcError[]; }
 ...
 * @ashActionType :read
 */
export async function readOptionSetWithDescendants<Fields extends ReadOptionSetWithDescendantsFields>(
  config: {
  tenant?: string;
  input: ReadOptionSetWithDescendantsInput;
  fields: Fields;
  filter?: OptionSetFilterInput;
  sort?: string;
  headers?: Record<string, string>;
...
): Promise<ReadOptionSetWithDescendantsResult<Fields>> {
  const payload = {
    action: "read_option_set_with_descendants",
    ...(config.tenant !== undefined && { tenant: config.tenant }),
    input: config.input,
    ...(config.fields !== undefined && { fields: config.fields }),
    ...(config.filter && { filter: config.filter }),
  ...
  return executeActionRpcRequest<ReadOptionSetWithDescendantsResult<Fields>>(
    payload,
    config
  );
}
 ...
 * @validation true
 */
export async function validateReadOptionSetWithDescendants(
  config: {
  tenant?: string;
  input: ReadOptionSetWithDescendantsInput;
  headers?: Record<string, string>;
